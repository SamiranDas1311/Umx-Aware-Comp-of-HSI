...
%   We aim to decompose the tensor X using orthogonal PARAFAC decomposition
...
%   P = OrthParafac(X,k,OPTS) specify options:
%   OPTS.tol: Tolerance on the difference in fit {1.0e-4}
...
%   OPTS.orthogonal: a vector (length = the number of modes of X)
%   OPTS.fix_mode: a logical vector (length = number of modes of X)
function [P, fit_info, Uinit] = OrthParafac(X, k, opts)
switch nargin
    case 2
        opts = struct;
    case 1
